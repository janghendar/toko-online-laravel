<script>
    (function ($) {
        ...
        $(document).on('submit', '.custom-form-builder-form', function (e) {
            ...
            $.ajax({
                ...
                beforeSend:function (){
                    btn.html(`<i class="las la-spinner la-spin mr-1"></i> <?php echo e(__('Submitting..')); ?>`);
                },
                processData: false,
                contentType: false,
                data:formData,
        ...
        $(document).on('click','.quick-view', function (e) {
            ...
            let stock_msg = "<?php echo e(__('Item is not available in stock')); ?>";
        ...
        $(document).ready(function () {
            ...
            $('.add_to_compare_ajax').on('click', function (e) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data) {
                            toastr.success('<?php echo e(__("Item added to compare")); ?>');
                        }
                    },
                    erorr: function (err) {
                        toastr.error('<?php echo e(__("Something went wrong")); ?>');
            ...
            $('#product_filter_featured_products').on('click', function (e) {
                let url = '<?php echo e(route("frontend.products.filter.top.rated")); ?>';
                loadFilterData(url);
            });

            $('#product_filter_top_selling').on('click', function (e) {
                let url = '<?php echo e(route("frontend.products.filter.top.selling")); ?>';
                loadFilterData(url);
            });

            $('#product_filter_new_products').on('click', function (e) {
                let url = '<?php echo e(route("frontend.products.filter.new")); ?>';
                loadFilterData(url);
            });
            ...
            $(document).on('keyup', '#search_form_input', function (e) {
                ...
                let btnOldText = "<?php echo e(__('Search')); ?>";
                if (!input_values.length) {
                    ...
                    $.get('<?php echo e(route('frontend.products.search')); ?>', {
                                    ...
                                    <a href="${product['url']}" class="item">
                                        ...
                                        <div class="product-info">
                                            ...
                                            <div class="product-price mt-2">
                                                ...
                                                <span class="stock-out">${product['is_stock'] ? '<?php echo e(__("In Stock")); ?>' : '<?php echo e(__("Stock Out")); ?>'}</span>
                                            </div>
                                        </div>
                                    </a>
                                </li>
            ...
            $('#select_site_currency').on('change', function () {
                ...
                $.post('<?php echo e(route('frontend.change.currency')); ?>', {
                    _token: '<?php echo e(csrf_token()); ?>',
            ...
            $('#change_site_language').on('change', function (e) {
                ...
                $.post('<?php echo e(route('frontend.change.language')); ?>', {
                    _token: '<?php echo e(csrf_token()); ?>',
                    language: language
                }).then(function (data) {
        ...
        function loadProductAttributeHtml(slug) {
            let url = "<?php echo e(route('frontend.products.attribute.html')); ?>";
            $('.lds-ellipsis').show();
            $.ajax({
                url: url,
                type: 'GET',
    ...
    $(document).on("click",".product_filter_style_two",function (){
        ...
        if($(this).attr("data-filter") == 'campaign'){
            send_ajax_response_get_response("post","<?php echo e(route("frontend.products.filter.campaign")); ?>");
        }else if($(this).attr("data-filter") == 'new-items'){
            send_ajax_response_get_response("post","<?php echo e(route("frontend.products.filter.new")); ?>");
        }else if($(this).attr("data-filter") == 'top-rated'){
            send_ajax_response_get_response("post","<?php echo e(route("frontend.products.filter.top.rated")); ?>");
        }else if($(this).attr("data-filter") == 'top-selling'){
            send_ajax_response_get_response("post","<?php echo e(route("frontend.products.filter.top.selling")); ?>");
        }else if($(this).attr("data-filter") == 'discounted'){
            send_ajax_response_get_response("post","<?php echo e(route("frontend.products.filter.discounted")); ?>");
        }
    ...
    function send_ajax_response_get_response(type,url){
        $.ajax({
            ...
            headers: {
                'X-CSRF-TOKEN': "<?php echo e(csrf_token()); ?>",//$('meta[name="csrf-token"]').attr('content')
            },
            beforeSend:function (){
...
<?php /**PATH /home/xgenxchi/public_html/laravel/grenmart/beta/@core/resources/views/frontend/partials/scripts/footer-scripts.blade.php ENDPATH**/ ?>