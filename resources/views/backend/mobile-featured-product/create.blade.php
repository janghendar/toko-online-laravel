                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route("admin.featured.product.create") }}" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input type="checkbox" id="category" name="category" {{ $selectedProduct->type == "category" ? "checked" : "" }}/>
                            </div>

                            <div class="form-group" id="product-list" {{ $selectedProduct->type == "category" ? "style=display:none" : "" }}>
                                <label for="products">Select Product</label>
                                <select id="products" name="featured_product[]" multiple class="form-control nice-select wide">
                                    <option value="">Select Product</option>
                                    @foreach($products as $product)
                                        <option {{ in_array($product->id,json_decode($selectedProduct->ids)) ? "selected" : "" }} value="{{ $product->id }}">{{ $product->name }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="form-group"  {{ $selectedProduct->type == "category" ? "" : "style=display:none" }} id="category-list">
                                <label for="products">Select Category</label>
                                <select id="products" name="featured_category[]" class="form-control">
                                    <option value="">Select Category</option>
                                    @foreach($categories as $category)
                                        <option {{ in_array($category->id,json_decode($selectedProduct->ids)) ? "selected" : "" }} value="{{ $category->id }}">{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
