...
class GeneralSettingsController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('permission:general-settings-navbar-category-dropdown', ['only', ['navbar_category_dropdown', 'update_navbar_category_dropdown']]);
    }
    ...
    public function update_license_settings(Request $request)
    {
        ...
        $response = Http::get('https://api.xgenious.com/license/new', [
            ...
            'site_url' => url('/'),
            'item_unique_key' => 'TQf1Ut2M2GVcn4q6S3t8jr31E6PddpNy',
        ]);
        $result = $response->json();
        $msg = __('your server could not able to connect with xgenious license server due to firewall, please contact support for license your script.');
        $type = 'danger';
        if($response->ok()){
    ...
    public function update_payment_settings(Request $request){
        $field_rules = [
            ...
            'instamojo_auth_token' => 'nullable|string|max:191',
            // marcadopago
        ...
        $env_val['INSTAMOJO_AUTH_TOKEN'] = $request->instamojo_auth_token ?? 'false';
    ...
    {
        $this->validate($request, ['global_navbar_variant' => 'nullable|string']);
     ...
     *              ]
     */
    public function update_navbar_category_dropdown(Request $request): \Illuminate\Http\RedirectResponse
    {
        $request->validate([
     ...
     */
    private function update_fields(Request $request, array $field_rules): \Illuminate\Http\RedirectResponse
    {
        $this->validate($request, $field_rules);
